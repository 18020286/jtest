...
def buildProject(buildType) {
	...
	def pomGroupId = readMavenPom([file: "pom.xml"]).getGroupId()
	def pomArtifactId = readMavenPom([file: "pom.xml"]).getArtifactId()
	def pomName = readMavenPom([file: "pom.xml"]).getName()
	def path = "target/auth-service-1.0.0.jar"
	packageServicesAndUploadToRepo(pomGroupId, pomArtifactId, pomName, path);

}

def packageServicesAndUploadToRepo(groupId, artifactId, moduleName, path){
	...
	stage('Upload artifact to Nexus server'){
		...
		nexusArtifactUploader   artifacts: [[ artifactId: "${artifactId}_${moduleName}",
											  ...
											  file: "${path}",
											  type: 'jar']],
				credentialsId: 'e96012f2-ff51-4a53-a398-c2f20f55e713',
				groupId: "${groupId}",
